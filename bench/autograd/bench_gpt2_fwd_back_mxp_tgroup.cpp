...
//
// GPT-2 MXP TensorGroup Benchmark
// Tests PersistentGPT2MXP with batched typecast via TypecastCache
//
// Compares:
// 1. BFP8: Inline typecast in backward (current)
// 2. MXP: Batched typecast before backward (optimized)
//
// Usage:
...
// BF16 benchmark (pure BF16 baseline)
template<size_t N>
...
// BFP8 benchmark (inline typecast - current implementation)
template<size_t N>
double benchmark_bfp8(uint32_t batch, uint32_t seq, uint32_t dim, uint32_t heads,
                      uint32_t ffn_mult, uint32_t vocab, float lr, MeshDevice& dev) {
...
// MXP benchmark (batched typecast via TypecastCache)
template<size_t N>
...
int main() {
    ...
    fmt::print("# GPT-2 Training Benchmark: BF16 vs BFP8 vs MXP\n");
    fmt::print("# Comparing: BF16 (baseline) vs BFP8 (inline typecast) vs MXP (batched typecast)\n");
    ...
    double bf16_ms = 0.0, bfp8_ms = 0.0, mxp_ms = 0.0;

    // Test 1: BF16 (baseline)
    fmt::print("## Test 1: BF16 - Pure BF16 baseline ({} layers)\n", num_layers);
    try {
        switch (num_layers) {
    ...
    // Test 2: BFP8 (inline typecast)
    fmt::print("## Test 2: BFP8 - Inline typecast ({} layers)\n", num_layers);
    try {
        switch (num_layers) {
            case 1: bfp8_ms = benchmark_bfp8<1>(batch, seq, dim, heads, ffn_mult, vocab, lr, device); break;
            case 2: bfp8_ms = benchmark_bfp8<2>(batch, seq, dim, heads, ffn_mult, vocab, lr, device); break;
            case 3: bfp8_ms = benchmark_bfp8<3>(batch, seq, dim, heads, ffn_mult, vocab, lr, device); break;
            case 6: bfp8_ms = benchmark_bfp8<6>(batch, seq, dim, heads, ffn_mult, vocab, lr, device); break;
        }
        fmt::print("BFP8: {:.3f} ms/step, {:.3f} ms/layer\n\n", bfp8_ms, bfp8_ms / num_layers);
    } catch (const std::exception& e) {
        fmt::print("BFP8 FAILED: {}\n\n", e.what());
        return 1;
    }

    // Test 3: MXP (batched typecast)
    fmt::print("## Test 3: MXP - Batched typecast ({} layers)\n", num_layers);
    try {
        switch (num_layers) {
            case 1: mxp_ms = benchmark_mxp<1>(batch, seq, dim, heads, ffn_mult, vocab, lr, device); break;
            case 2: mxp_ms = benchmark_mxp<2>(batch, seq, dim, heads, ffn_mult, vocab, lr, device); break;
            case 3: mxp_ms = benchmark_mxp<3>(batch, seq, dim, heads, ffn_mult, vocab, lr, device); break;
            case 6: mxp_ms = benchmark_mxp<6>(batch, seq, dim, heads, ffn_mult, vocab, lr, device); break;
        }
    ...
    double bfp8_tflops = (model_flops / 1e12) / (bfp8_ms / 1000.0);
    double mxp_tflops = (model_flops / 1e12) / (mxp_ms / 1000.0);
    double bfp8_vs_bf16 = bf16_ms / bfp8_ms;
    double mxp_vs_bf16 = bf16_ms / mxp_ms;

    // Summary
    fmt::print("## Summary\n");
    fmt::print("| Mode | Time (ms) | TFLOPS | vs BF16 |\n");
    fmt::print("|------|-----------|--------|--------|\n");
    fmt::print("| BF16 (baseline) | {:.3f} | {:.2f} | 1.00x |\n", bf16_ms, bf16_tflops);
    fmt::print("| BFP8 (inline typecast) | {:.3f} | {:.2f} | {:.2f}x |\n", bfp8_ms, bfp8_tflops, bfp8_vs_bf16);
    fmt::print("| MXP (batched typecast) | {:.3f} | {:.2f} | {:.2f}x |\n", mxp_ms, mxp_tflops, mxp_vs_bf16);
    fmt::print("\n");

    // Analysis
    fmt::print("## Analysis\n");
    if (bfp8_vs_bf16 > 1.05) {
        fmt::print("BFP8 provides {:.1f}% speedup over BF16\n", (bfp8_vs_bf16 - 1.0) * 100);
    } else if (bfp8_vs_bf16 < 0.95) {
        fmt::print("BFP8 is {:.1f}% SLOWER than BF16 (typecast overhead)\n", (1.0 - bfp8_vs_bf16) * 100);
    } else {
        fmt::print("BFP8 ~same as BF16\n");
    }

    if (mxp_vs_bf16 > 1.05) {
        fmt::print("MXP provides {:.1f}% speedup over BF16\n", (mxp_vs_bf16 - 1.0) * 100);
    } else if (mxp_vs_bf16 < 0.95) {
        fmt::print("MXP is {:.1f}% SLOWER than BF16\n", (1.0 - mxp_vs_bf16) * 100);
    } else {
        fmt::print("MXP ~same as BF16\n");
    }

    fmt::print("\n# CSV: batch,seq,dim,heads,vocab,layers,bf16_ms,bfp8_ms,mxp_ms\n");
    fmt::print("# {},{},{},{},{},{},{:.3f},{:.3f},{:.3f}\n",
               batch, seq, dim, heads, vocab, num_layers, bf16_ms, bfp8_ms, mxp_ms);
