...
//
// Matmul benchmark on 2-device MeshDevice (1x2) for n300
...
BenchResult benchmark_matmul(MeshDevice& device, uint32_t size, int num_devices) {
    ...
    // TFLOP/s: num_devices * 2 * N^3 / time
...
int main(int argc, char* argv[]) {
    // Accept device ID from command line, default to 0
    int device_id = 0;
    if (argc > 1) {
    ...
    fmt::print("# Testing {} sizes on device {}\n", sizes.size(), device_id);
    fmt::print("device,size,mean_us,std_us,min_us,max_us,tflops\n");

    for (uint32_t size : sizes) {
        try {
            auto result = benchmark_matmul(*device, size, 1);
            fmt::print("{},{},{:.2f},{:.2f},{:.2f},{:.2f},{:.3f}\n",
                       device_id, result.size, result.mean_us, result.std_us,
                       result.min_us, result.max_us, result.tflops);
        } catch (const std::exception& e) {
            fmt::print(stderr, "# Error at device {} size {}: {}\n", device_id, size, e.what());
        }
    }
