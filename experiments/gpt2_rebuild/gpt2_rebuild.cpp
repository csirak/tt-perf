...
//
// GPT-2 rebuild for incremental parity testing (single layer, BF16).
...
struct Config {
    std::string data_dir = "/home/howard/ttnn-perf/experiments/gpt2_rebuild/data";
    std::string outputs_dir = "/home/howard/ttnn-perf/experiments/gpt2_rebuild/outputs";
    uint32_t batch = 32;
    uint32_t seq = 32;
    uint32_t dim = 128;
    uint32_t heads = 4;
...
int main() {
    const char* env_path = std::getenv("GPT2_REBUILD_CONFIG");
    std::string config_path = env_path
        ? env_path
        : "/home/howard/ttnn-perf/experiments/gpt2_rebuild/default.yaml";
