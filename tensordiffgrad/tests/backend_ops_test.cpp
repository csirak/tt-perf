...
//
// tensordiffgrad backend ops test (single readable file).
...
int main() {
    ...
    print_result(compare_tensors_full("add", add_out, ref_add));
    print_result(compare_tensors_full("sub", sub_out, ref_sub));
    print_result(compare_tensors_full("mm", mm_out, ref_mm));
    ...
    print_result(compare_tensors_full("mm_fp32_acc", mm_cfg_out, ref_mm));
#endif
