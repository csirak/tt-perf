...
//
// GPT-2 train rebuild for incremental parity testing (single layer, BF16).
...
struct Config {
    std::string data_dir = "/home/howard/ttnn-perf/train/gpt2/data";
    std::string outputs_dir = "/home/howard/ttnn-perf/train/gpt2/outputs";
    uint32_t batch = 32;
    uint32_t seq = 32;
    uint32_t dim = 128;
    uint32_t heads = 4;
...
int main() {
    const char* env_path = std::getenv("TRAIN_GPT2_CONFIG");
    std::string config_path = env_path
        ? env_path
        : "/home/howard/ttnn-perf/train/gpt2/default.yaml";

    Config cfg = load_config(config_path);

    fmt::print("# GPT2 train rebuild config: {}\n", config_path);
    ...
    for (uint32_t step = 0; step < cfg.steps; ++step) {
...

        {
            ...
            loss_v = mse(graph, out_v, &target, &loss, &d_loss, &diff);
