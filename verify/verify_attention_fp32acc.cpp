...
//
// Attention forward pass verification with FP32 ACCUMULATION ENABLED.
// Ablation study to compare accuracy vs default BF16 accumulation.
//
// Usage:
//   make verify-attention-fp32acc
...
const std::string OUTPUT_DIR = "verify/outputs/cpp_fp32acc";

int main() {
    fmt::print("======================================================================\n");
    fmt::print("C++ Attention Forward Verification (FP32 ACCUMULATION)\n");
    fmt::print("======================================================================\n");
    fmt::print("Config: batch={}, seq={}, dim={}, heads={}\n", BATCH, SEQ, DIM, HEADS);
    fmt::print("FP32 dest accumulation: ENABLED\n");
    fmt::print("Input dir: {}\n", INPUT_DIR);
    ...
    // FP32 accumulation compute config for matmul operations
    auto compute_config = ttnn::WormholeComputeKernelConfig{
        .math_fidelity = MathFidelity::HiFi2,
        .math_approx_mode = false,
        .fp32_dest_acc_en = true,   // KEY: Enable FP32 accumulation
        .packer_l1_acc = true,
    };
    fmt::print("Compute config: HiFi2, fp32_dest_acc_en=true, packer_l1_acc=true\n\n");
    ...
    try {
        ...
        // Forward pass with FP32 accumulation
        fmt::print("\nRunning forward pass (FP32 acc)...\n");

        // QKV projections: y = x @ W.T (with FP32 acc)
        ...
        // Attention scores: Q @ K.T * scale (with FP32 acc)
        float scale = 1.0f / std::sqrt(static_cast<float>(HEAD_DIM));
        auto scores = ttnn::matmul(q, k, false, true, std::nullopt, std::nullopt,
                                   std::nullopt, std::nullopt, compute_config);
        ...
        // Attention output: weights @ V (with FP32 acc)
        ...
        // Output projection (with FP32 acc)
